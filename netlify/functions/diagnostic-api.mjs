// diagnostic-api.mjs — Netlify Functions v2
// Advisory Diagnostic Tool — BW Advisory Solutions
// Handles: submit-diagnostic (POST), approve/reject (GET with token)
...
export default async (req, context) => {
  ...
  const functionBase = `${reqUrl.origin}/.netlify/functions/diagnostic-api`;
  ...
  if (req.method === 'GET') {
    ...
    if (!action || !token) {
      return htmlResponse(brandedPage('Invalid Request', '<p>This link is missing required parameters.</p>'), 400);
    }
    ...
    try {
      ...
      return htmlResponse(brandedPage('Error', '<p>Unable to retrieve submission. Please try again.</p>'), 500);
    }
    ...
    if (action === 'approve') {
      if (submission.status === 'sent') {
        ...
        return htmlResponse(brandedPage('Already Sent', `<p>This report was already sent to <strong>${escapeHtml(submission.prospect.name)}</strong> at <strong>${escapeHtml(submission.prospect.email)}</strong> on ${sentDate}.</p>`));
      }
      ...
      try {
        await sendProspectEmail(submission);
        submission.status = 'sent';
        submission.sentAt = new Date().toISOString();
        await store.setJSON(`sub_${subId}`, submission);
        return htmlResponse(brandedPage(
          'Report Sent',
          `<p style="font-size:17px;line-height:1.7;">Report sent to <strong>${escapeHtml(submission.prospect.name)}</strong><br>at <strong>${escapeHtml(submission.prospect.email)}</strong>.</p>
          <p style="margin-top:16px;font-size:14px;color:#555;">Submission ID: ${escapeHtml(subId)}</p>`
        ));
      } catch (e) {
        console.error('Prospect email send failed:', e.message);
        return htmlResponse(brandedPage('Send Failed', `<p>Failed to send the report: ${escapeHtml(e.message)}</p><p style="margin-top:12px;">Please retry or send manually to ${escapeHtml(submission.prospect.email)}.</p>`), 500);
      }
    }

    if (action === 'reject') {
      if (submission.status !== 'pending') {
        return htmlResponse(brandedPage('Already Actioned', `<p>This submission has already been ${escapeHtml(submission.status)}. No further action required.</p>`));
      }
      ...
      try {
        ...
        return htmlResponse(brandedPage('Error', `<p>Failed to update submission status: ${escapeHtml(e.message)}</p>`), 500);
      }
      return htmlResponse(brandedPage('Submission Rejected', `<p>Submission from <strong>${escapeHtml(submission.prospect.name)}</strong> at ${escapeHtml(submission.prospect.organisation)} has been rejected. No report will be sent.</p>`));
    }
  ...
  if (req.method === 'POST') {
    ...
    const diagType = type === 'operational' ? 'operational' : 'strategic';

    if (!prospect?.name?.trim() || !prospect?.email?.trim() ||
        !prospect?.organisation?.trim() || !Array.isArray(answers) || !answers.length) {
      return jsonResponse({ error: 'Missing required fields: prospect.name, prospect.email, prospect.organisation, answers' }, 400);
    }
    ...
    // Generate report and notify Brad after the response is sent —
    // context.waitUntil keeps the function alive without blocking the client
    context.waitUntil((async () => {
      ...
      try {
        await notifyBrad(submission, functionBase);
      } catch (e) {
...
async function generateReport(prospect, answers, diagType = 'strategic') {
  ...
  let systemPrompt = `You are a senior management consultant with 30 years of advisory experience spanning law enforcement, government, and the private sector. You specialise in closing the gap between strategic intent and operational execution.

You have received a self-assessment from a senior leader. Your task is to produce a structured diagnostic report. Write as a human expert who has carefully read and synthesised these responses. Never reference AI, algorithms, software, or automation in your analysis.
...
- No sentence may begin with "There is", "There are", "There was", or "There were". Restructure to lead with the subject.
- Active voice by default.
- Every sentence earns its place. If a word adds nothing, cut it.
...
{
  "headline": "A single sharp sentence capturing the core strategic challenge facing this organisation. Should name the organisation or leader specifically. Example: 'The constraint at Acme Corp is not strategy — it is the gap between direction set at the top and behaviour embedded at the front line.'",
  "section1_findings": [
    "Finding 1 — a direct, specific observation drawn from their responses. One sentence. Grounded in what they actually said.",
    "Finding 2 — a second observation. Focus on a different dimension.",
    "Finding 3 — a third observation. Draw out a tension or implication they may not have named."
  ],
  "section2_primary": "The single primary constraint preventing progress. Name it plainly. One to two sentences. Specific to their situation.",
  "section2_secondary": [
    "Secondary constraint 1 — a contributing factor beneath or alongside the primary constraint.",
    "Secondary constraint 2 — a second contributing factor."
  ],
  "section2_hypothesis": "A root cause hypothesis. Two to three sentences. What is driving the primary constraint? What mechanism connects strategy to the breakdown at the operational level? Be precise — name what is likely happening and why.",
  "section3_steps": [
    "Priority area 1 — describe the first area where the gap cannot be closed without structured, expert-led work. Name what makes this hard and why it resists a straightforward internal fix. Do not tell the client what to go and do — describe the nature and complexity of the work required.",
    "Priority area 2 — the logical follow-on area. Explain why this compounds the first. Frame in terms of what structured attention to this area would need to examine and address."
  ],
  "section3_close": "A closing paragraph. Two to three sentences. Acknowledge the depth of what these gaps represent — not just process problems but embedded organisational patterns. Note that establishing the full picture and a credible path forward is the work of a structured engagement, not a self-directed fix. Keep this direct — name the reality, do not sell.",
  "bw_alignment": [
    "Systems & Process Optimisation — one specific sentence connecting this service line to something observed in their assessment, OR omit this entry entirely if not genuinely relevant.",
    "Technology Integration & Capability Deployment — one specific sentence if relevant, OR omit.",
    "Organisational Design & Change Management — one specific sentence if relevant, OR omit.",
    "Strategic Advisory & Programme Leadership — one specific sentence if relevant, OR omit."
  ]
}

Only include service lines that are directly relevant to this organisation's situation. Do not include generic descriptions — every entry must reference something specific from their responses. Include two to four entries.

Return only the JSON object. No preamble, no explanation.`;

  // Operational type overrides
  if (diagType === 'operational') {
    systemPrompt = `You are a senior management consultant with 30 years of operational advisory experience spanning law enforcement, government, and the private sector. You specialise in identifying where organisations fail to convert strategy into reliable, consistent operational performance.\n\nYou have received an operational self-assessment from a senior leader. Produce a structured diagnostic report. Write as a human expert who has carefully read and synthesised these responses. Never reference AI, algorithms, software, or automation in your analysis.\n\nWriting rules — non-negotiable:\n- Direct, senior-consulting register. No marketing language, no waffle.\n- Australian English: organisation, programme, behaviour, colour, recognise.\n- No sentence may begin with "There is", "There are", "There was", or "There were". Restructure to lead with the subject.\n- Active voice by default.\n- Every sentence earns its place. If a word adds nothing, cut it.\n- Specific to this person's situation — no generic observations.`;
    userPrompt = `Respondent: ${prospect.name}, ${prospect.role}, ${prospect.organisation}\n\nASSESSMENT RESPONSES:\n${answersBlock}\n\nProduce a diagnostic report as a JSON object with this exact structure. All string values must be plain text (no HTML, no markdown):\n\n{\n  "headline": "A single sharp sentence capturing the core operational challenge facing this organisation. Should name the organisation or leader specifically. Example: 'The breakdown at Acme Corp is not a resource problem — it is a consistency problem, driven by unclear accountability at the point of execution.'",\n  "section1_findings": [\n    "Finding 1 — a direct, specific observation drawn from their responses. One sentence. Grounded in what they actually said.",\n    "Finding 2 — a second observation. Focus on a different operational dimension.",\n    "Finding 3 — a third observation. Draw out a tension or implication they may not have named explicitly."\n  ],\n  "section2_primary": "The single primary operational constraint preventing performance improvement. Name it plainly. One to two sentences. Specific to their situation.",\n  "section2_secondary": [\n    "Secondary constraint 1 — a contributing factor beneath or alongside the primary constraint.",\n    "Secondary constraint 2 — a second contributing factor."\n  ],\n  "section2_hypothesis": "A root cause hypothesis. Two to three sentences. What is driving the primary constraint? What mechanism is creating the operational friction or inconsistency? Be precise — name what is likely happening and why.",\n  "section3_steps": [\n    "Priority area 1 — describe the first area where the operational gap cannot be closed without structured, expert-led work. Name what makes this hard and why it resists a straightforward internal fix. Do not tell the client what to go and do — describe the nature and complexity of the work required.",\n    "Priority area 2 — the logical follow-on area. Explain why this compounds the first. Frame in terms of what structured attention to this area would need to examine and address."\n  ],\n  "section3_close": "A closing paragraph. Two to three sentences. Acknowledge the depth of what these gaps represent — not just process problems but embedded operational patterns. Note that establishing the full picture and a credible path forward is the work of a structured engagement, not a self-directed fix. Keep this direct — name the reality, do not sell.",\n  "bw_alignment": [\n    "Systems & Process Optimisation — one specific sentence connecting this service line to something observed in their assessment, OR omit this entry entirely if not genuinely relevant.",\n    "Technology Integration & Capability Deployment — one specific sentence if relevant, OR omit.",\n    "Organisational Design & Change Management — one specific sentence if relevant, OR omit.",\n    "Strategic Advisory & Programme Leadership — one specific sentence if relevant, OR omit."\n  ]\n}\n\nOnly include service lines that are directly relevant to this organisation\'s situation. Do not include generic descriptions — every entry must reference something specific from their responses. Include two to four entries.\n\nReturn only the JSON object. No preamble, no explanation.`;
  }
  ...
  // Extract JSON (Claude may occasionally add ``` fences)
...
<title>${diagType === 'operational' ? 'Operational' : 'Strategic'} Diagnostic Assessment — ${e(prospect.organisation)}</title>
</head>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:660px;background:#ffffff;border-radius:8px;overflow:hidden;">
  ...
  <tr><td style="padding:40px 48px;">
    ...
    <tr><td style="padding:0 48px 32px;">
      <div style="background:#fff8ec;border:1px solid #f0d9a0;border-radius:6px;padding:14px 18px;">
        <span style="font-size:12px;font-weight:700;color:#6b4f1a;">Important: </span><span style="font-size:12px;color:#6b4f1a;line-height:1.6;">This report is provided for informational purposes only and is based solely on the responses submitted. It does not constitute professional, legal, financial, or strategic advice. BW Advisory Solutions makes no warranty as to the accuracy, completeness, or suitability of this report to your circumstances. No liability is accepted for any decisions made or actions taken in reliance on this report. Receipt of this report does not create a client relationship or any obligation on either party.</span>
      </div>
    </td></tr>
...
async function notifyBrad(submission, functionBase) {
  const { prospect, answers, id, token, report } = submission;
  const transporter = getTransporter();

  const approveUrl = `${functionBase}?action=approve&token=${encodeURIComponent(token)}`;
  const rejectUrl = `${functionBase}?action=reject&token=${encodeURIComponent(token)}`;
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:720px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;">
  ...
  await transporter.sendMail({
    ...
    to: BRAD_EMAIL,
    subject: `[${submission.diagType === 'operational' ? 'Operational' : 'Strategic'} Diagnostic] ${prospect.name} — ${prospect.organisation}`,
    html: emailHtml,
  });
}
